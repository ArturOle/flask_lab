...
#         return books
    ...
    def indexPost(self):
        if 'user' in session:
            ...
            if response_form["author"] != "" and response_form["title"] != "":
                ...
                cur = con.cursor()
                cur.execute("SELECT * FROM books where author = ? and title = ? LIMIT 1", \
                    (response_form['author'], response_form['title']))
                books = cur.fetchall()
                if books == []:
                    cur.execute("INSERT INTO books (author,title) VALUES (?,?)", \
                        (response_form["author"],response_form["title"]))
                    con.commit()
                    con.close()
                    books = self.load_books()
                    redirect("/")
                    return render_template('index.html', books = books, message = "book added", admin = admin)
                else:
                    books = self.load_books()
                    redirect("/")
                    return render_template('index.html', books = books, message = "book already in the database!", admin = admin)
            else:
                books = self.load_books()
                redirect("/")
                return render_template('index.html', books = books, message = "author and title cannot be empty!", admin = admin)
        else:
            return redirect("/login")
    ...
    def loginPost(self):
        if 'user' not in session:
            ...
            con = sqlite3.connect(self.DATABASE)
            cur = con.cursor()
            cur.execute("SELECT * FROM users where username=? and password=? LIMIT 1", (response_form['login'], response_form['password']))
            user = cur.fetchall()
            if user == []:
        ...
        if 'user' in session:
            if admin == 1:
                ...
                if response_form["login"] != "" and response_form["password"] != "":
                    ...
                    cur = con.cursor()
                    print(response_form)
                    cur.execute("SELECT * FROM users where username = \"" +str(response_form["login"]) + "\" LIMIT 1")
                    users = cur.fetchall()
                    if users == []:
                        try:
